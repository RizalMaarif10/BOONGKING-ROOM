...
@section('page_subtitle', 'Ubah data ruangan yang sudah ada')
                    ...
                    <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST">
                            ...
                            <select name="jenis_ruangan_id"
                                ...
                                @foreach ($jenisRuangan as $jenis)
                                    <option value="{{ $jenis->id }}"
                                        {{ old('jenis_ruangan_id', $room->jenis_ruangan_id) == $jenis->id ? 'selected' : '' }}>
                                        {{ $jenis->nama }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="fasilitas" rows="3"
                                class="form-control mt-1 @error('fasilitas') is-invalid @enderror"
                                style="font-size:0.85rem;border-radius:10px;">{{ old('fasilitas', $room->fasilitas) }}</textarea>
                            @error('fasilitas')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
